...
export function iplAuctionSummary(team, players) {
  if (!team || typeof team !== 'object' || !team.name || !team.purse || team.purse <= 0) return null;
  if (!Array.isArray(players) || players.length === 0) return null;

  const totalSpent = players.reduce((acc, player) => acc + player.price, 0);
  const remaining = team.purse - totalSpent;
  const playerCount = players.length;
  const costliestPlayer = players.reduce((acc, player) => player.price > acc.price ? player : acc, players[0]);
  ...
  const byRole = players.reduce((acc, player) => {
    if (!acc[player.role]) acc[player.role] = 0;
    acc[player.role]++;
    return acc;
  }, {});
